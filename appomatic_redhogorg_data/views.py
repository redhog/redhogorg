...
def node(request, url):
    ...
    if url == "" or url == "/":
        return django.http.HttpResponse(appomatic_redhogorg_data.models.Node.render_list(request, style=style))
    else:
...
def tag(request, name):
    ...
    if name == "" or name == "/":
        return django.http.HttpResponse(appomatic_redhogorg_data.models.Tag.render_list(request, style=style))
    else:
        return django.http.HttpResponse(appomatic_redhogorg_data.models.Tag.objects.get(name=urllib.unquote_plus(name).decode('utf8')).render(request, style=style))
